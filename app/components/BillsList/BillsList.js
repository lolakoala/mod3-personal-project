...
class BillsList extends Component {
  ...
  getUserBills = bills => {
    const { currentUser, usersHouse, markBillPaid } = this.props;
    return bills.map(bill => {
      ...
      return (<div key={bill.parsedDuedate}>
        ...
        <p>{bill.duedate}</p>
        <p>{user.total}</p>
        <div onClick={() => markBillPaid(bill.id, currentUser.id, usersHouse)}>{user.paid ? 'Paid' : 'Mark as Paid'}</div>
      </div>);
    });
  }
    ...
    if (billsToMap.length) {
      return (
        <div>
          ...
          {placeRendered === '/houselist' ? this.getHouseBills(billsToMap) : this.getUserBills(billsToMap)}
        </div>
      );
    } else {
      return (
